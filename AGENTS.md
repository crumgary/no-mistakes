...
- That `GIT_CONFIG_COUNT` unset does not cover a contributor's real global `~/.gitconfig`: `internal/pipeline/steps`' `gitCmd` test helper (`helpers_test.go`) forwards `os.Environ()`, including `HOME`, into every real `git` invocation it makes. A global `rebase.autostash = true` (or similar) silently changes rebase behavior and fails the non-conflict-rebase-failure tests (e.g. `TestRebaseStep_NonConflictFailureWithRebaseMetadataReturnsError`) without any code being wrong. If you hit that failure, check `git config --global --get rebase.autostash` before assuming a regression.
